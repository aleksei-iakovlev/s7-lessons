...
def input_event_paths(date, depth):
    ...
    return [f"/user/s19290263/data/events/date={(dt - datetime.timedelta(days=x)).strftime('%Y-%m-%d')}" for x in
            range(depth)]
...
def reaction_tag_tops(date, depth, spark):
    ...
    all_message_tags = spark.read.option("mergeSchema", "true").parquet("/user/s19290263/data/events")\
    ...
    print("here 1 ")
    result.write.mode("overwrite").parquet(f"/user/s19290263/data/tmp/reaction_tag_tops_{date[5:7]}_{date[8:10]}_{depth}")
    return None

# reaction_tag_tops('2022-05-31', 7, spark)
depths=[5,5,1]
